...
if [[ $(tmux has-session -t "gopro-process") == 0 ]]; then
    ...
    tmux send-keys -t "gopro-process" "sudo gopro webcam -n -p enp* && \
        tmux wait-channel -S \"gopropwd\" && \
        ffmpeg -nostdin -threads 1 -i 'udp://@0.0.0.0:8554?overrun_nonfatal=1&fifo_size=50000000' -f:v mpegts -fflags nobuffer -vf format=yuv420p -f v4l2 /dev/video42" C-m
    tmux attach-session -t "gopro-process"
    tmux wait-channel -S "gopropwd"
    if [[ tmux capture-pane -p -t "session_name:window.pane" | grep "Error while starting the Webcam mode" ]]; then
...
tmux kill-session -t ros2-xarm 2>/dev/null; tmux new -s ros2-xarm

SOURCE_COMMANDS="source /opt/ros/humble/setup.bash && source install/setup.bash"

# launch gopro node
tmux new-window -t ros2-xarm -n "gopro"
...
tmux new-window -t ros2-xarm -n "planning"
...
# launch control node
tmux new-window -t ros2-xarm -n "controller"
...
tmux new-window -t ros2-xarm -n "weed"
